...
- [x] 09-01: Deep-merge `enabledSources` in `getSettings` (background) and dashboard settings load

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 09-02: Settings migration to backfill missing source flags on load

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 09-03: Unit-test regression coverage for the deep-merge + migration fix
- [x] 09-04: Gap closure for deep-merge + migration regression coverage
...
**Goal**: Surface important topics even with an empty watchlist by detecting when the same topic appears across >=3 distinct source types (mixing social + news), reusing the existing `newsSocialMatches` correlation output and the shared alert infrastructure.
...
- [x] 10-01: End-to-end cross-source consensus alert engine (tracer) + AlertsTab rendering

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 10-02: Consensus edge cases (distinct source counting, social/news mix, any-direction, cooldown, gating)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 10-03: Kind-aware AlertsTab UI + full regression verification
