# Requirements Archive: v0.1.6 v0.1.6
...
- [x] **TRIG-01**: User's correlation results carry freshness metadata — every terminal path (success, ML error, cancel) writes `CONFIG.storage.correlations` including `computedAt` (+ input metadata); error results never clobber fresh good cached results
- [x] **TRIG-02**: User sees cached results instantly on tab open — no auto-analyze on dashboard load; analysis runs only when no stored analysis exists
- [x] **TRIG-03**: User gets fresh correlations after collecting — re-analyze triggers when collectNow/collection completes (via `storage.onChanged` on snapshot keys, mirroring the `useSnapshot` pattern)
- [x] **TRIG-04**: User can tell live from cached results — Correlations header shows `computedAt` + engine (stale-result badge)
...
| MLPROG-03 | Deferred (v0.1.7+) | Pending |
| TRIG-01 | Phase 16 | Complete |
| TRIG-02 | Phase 16 | Complete |
| TRIG-03 | Phase 16 | Complete |
| TRIG-04 | Phase 16 | Complete |
| TRIG-05 | Deferred (v0.1.7+) | Pending |

**Coverage:**

- v0.1.6 requirements: 12 total (10 mapped to phases + 2 deferred)
- Mapped to phases: 10
- Deferred: 2 (CORR-05, CORR-06, MLPROG-03, TRIG-05 — see Deferred section)
- Unmapped: 0 ✓

---
*Requirements defined: 2026-08-27*
*Last updated: 2026-08-30 — Phase 16 complete; MLPROG-03/TRIG-05 added to traceability*
