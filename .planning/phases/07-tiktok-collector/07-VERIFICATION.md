...
verified: 2026-08-23T16:55:00Z
status: human_needed
score: 1/4 must-haves verified
behavior_unverified: 3
overrides_applied: 0
behavior_unverified_items:
  - truth: "User can see TikTok social sentiment in the dashboard when the discover page is reachable"
    test: "Enable TikTok in the popup and trigger a collection cycle; confirm a background tab opens to the discover page and 🎵 TikTok signals appear in the dashboard HypeFeed"
    expected: "Scraped trends normalize to SocialSignal (platform 'tiktok') and render in HypeFeed with the 🎵 icon"
    why_human: "TikTok's discover page is a heavily-obfuscated SPA; broad selectors may or may not match the live DOM. Presence + wiring prove the code path exists, not that scraping yields real signals."
  - truth: "User sees a graceful degradation state (source health indicator) when TikTok is unavailable; TikTok collects automatically (no URL-paste input)"
    test: "With TikTok unreachable, confirm the 🎵 TikTok badge shows 'No data'/'Degraded'; confirm TikTok is collected automatically via a background tab (no URL-paste input box in the dashboard)"
    expected: "Badge reflects no-data/degraded state; automatic background-tab collection opens the discover page, scrapes, reports, and closes the tab"
    why_human: "Badge rendering and automatic background-tab collection are real-browser behaviors (tabs.create, onMessage from a real tab) that unit tests with mocked browser cannot exercise."
  - truth: "User can see additional data sources beyond TikTok wired end-to-end"
    test: "Open the dashboard and export; confirm 6 news sources + Polymarket/Kalshi + Reddit/X + TikTok all render and export"
    expected: "All sources render in dashboard and export with correct platform/source columns"
    why_human: "End-to-end rendering/export across all sources is a browser-level integration check; code-level wiring is present but not browser-verified."
human_verification:
  - test: "Enable TikTok in the popup and trigger a collection cycle; confirm a background tab opens to the discover page and 🎵 TikTok signals appear in the dashboard HypeFeed"
    expected: "Scraped trends normalize to SocialSignal (platform 'tiktok') and render in HypeFeed with the 🎵 icon"
    why_human: "TikTok's discover page is a heavily-obfuscated SPA; broad selectors may or may not match the live DOM. Presence + wiring prove the code path exists, not that scraping yields real signals."
  - test: "With TikTok unreachable, confirm the 🎵 TikTok badge shows 'No data'/'Degraded'; confirm TikTok is collected automatically via a background tab (no URL-paste input box)"
    expected: "Badge reflects no-data/degraded state; automatic background-tab collection opens the discover page, scrapes, reports, and closes the tab"
    why_human: "Badge rendering and automatic background-tab collection are real-browser behaviors (tabs.create, onMessage from a real tab) that unit tests with mocked browser cannot exercise."
  - test: "Open the dashboard and export; confirm 6 news sources + Polymarket/Kalshi + Reddit/X + TikTok all render and export"
    expected: "All sources render in dashboard and export with correct platform/source columns"
